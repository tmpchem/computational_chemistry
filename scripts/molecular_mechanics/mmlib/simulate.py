...
class Simulation:
    ...
    Derives into MolecularDynamics and MonteCarlo. Contains
    ...
    Attributes:
        ...
        simtype (str): Type of simulation (set through constructors
            of derived classes). Values:
        ...
        statustime (float): Clock time between printing status to
            standard output [s].
        random_seed (int): Random number generator seed for initial
            velocity assignment / displacement (random uint32 by default).
    ...
    def print_geom(self):
        ...
        fileio.print_coords_file(self.gfile, self.mol, pstr,
            self.gprintchar, self.gprintdig)
    ...
    def print_energy(self):
        ...
        self.print_val(self.eprintchar+2, self.eprintdig+2,
            self.mol.e_total, 'e')
    ...
    def print_status(self):
        ...
        if (self.simtype == 'md'):
            print('%.*f/%.*f ps' % (self.tprintdig, self.time,
                self.tprintdig, self.tottime), end='')
        elif (self.simtype == 'mc'):
...
class MolecularDynamics(Simulation):
    ...
    Molecular dynamics takes an initial geometry, randomly assigns
    velocities, and propogates system forward in time for a specified
    amount of time through numerical integration of Newton's equations.
    
    Args:
        infile_name (str): Path of simulation input file. May be relative
            or absolute, though absolute is safer.
...
class MonteCarlo(Simulation):
    ...
    Metropolis Monte-Carlo generates a set of configurations for a system
    by propogating an initial configuration forward in time through
    random displacements of coordinates which are accepted or rejected
    based on the relative potential energy Boltzmann factor.
